...
order <- c(6,7,5,11,14,13,12,10,4,8,1,2,3,9,15)
...
Classes <- set$class
colours_classes <- c("Blank" = "blue", "PCYP" = "red", 
                     "QC" = "black")
...
matrix.normalised <- matrix.transformed 
...
if (length(matrix) > 1) 
  {
    ...
    heatmap.2(x = t(matrix), scale = "row", distfun = dist, margins = c(15,15), trace = "none")

    ## Perform t-SNE analysis and plot results
    tsne <- Rtsne(matrix, dim = 2, perplexity = 4, epoch = 100,
...
significant.features$mz <- rbind(peaklist[which(peaklist$QC == 5),], peaklist[which(peaklist$QC != 5),])$mz
significant.features$rt <- rbind(peaklist[which(peaklist$QC == 5),], peaklist[which(peaklist$QC != 5),])$rt
significant.features$adduct <- rbind(getPeaklist(set.CAMERA)[which(getPeaklist(set.CAMERA)$QC == 5),],
                                     getPeaklist(set.CAMERA)[which(getPeaklist(set.CAMERA)$QC != 5),])$adduct
significant.features$isotopes <- rbind(getPeaklist(set.CAMERA)[which(getPeaklist(set.CAMERA)$QC == 5),],
                                     getPeaklist(set.CAMERA)[which(getPeaklist(set.CAMERA)$QC != 5),])$isotopes
significant.features$pcgroup <- rbind(getPeaklist(set.CAMERA)[which(getPeaklist(set.CAMERA)$QC == 5),],
                                      getPeaklist(set.CAMERA)[which(getPeaklist(set.CAMERA)$QC != 5),])$pcgroup
...
write.csv(significance.result, file = "evaluateResults/significant_result.csv")