#########################################################################
#                                                                        #
# Use this script for an initial optimization approach for your          #
# peak picking algorithm!                                                # 
# In a first step the parameters will be used in a certain range         #
# specified in lines 38-44 and replace the XCMS-Online Q-Exactive preset #
# parameters one by one!                                                 #
# In a second step the top three parameters (except prefilter 1 and 2)   #
# will be combined in every possible way and the process will be         #
# repeated!                                                              #
#                                                                        #
# After you obtained the CSV files this from script use plotSet.R to     #
# evaluate your results and manually adjust the parameters in plausible  # 
# ranges obtained by the single Result.csv files                         #
#                                                                        #
# Place XCMS readable files in your working directory and enter          # 
# your file extension!                                                   #
  format <- ".mzXML"
#                                                                        #
# Note that this script creates a folder in your working directory       #
# (see line 43)!                                                         #
#                                                                        #
# This script requires tidyverse, stringr, parallel,                     # 
# BiocParallel and XCMS!                                                 #
# If not installed you can use the following lines to install them:      #
# install.packages(c("tidyverse", "stringr", "parallel"))                #
# source("https://bioconductor.org/biocLite.R")                          #
# biocLite("xcms", "BiocParallel")                                       # 
#                                                                        #
##########################################################################
...
#Set parameters for multicore computing
params <- MulticoreParam(workers = detectCores() -1)
...
bandwidth <- seq(from = 0.5, to = 5, by = 0.5)

#Create one matrices with preset parameters for every paramter to optimise
parameters.peakwidth.min <- matrix(data = preset, ncol = 8, nrow = length(peakwidth.min), byrow = TRUE, dimnames = list(NULL, 
                                   c("Minimum Peakwidth", "Maximum Peakwidth", "ppm", "S/N Threshold", "m/z Difference", "Prefilter 1", "Prefilter 2", "bandwidth")))

parameters.peakwidth.max <- matrix(data = preset, ncol = 8, nrow = length(peakwidth.max), byrow = TRUE, dimnames = list(NULL, 
                                   c("Minimum Peakwidth", "Maximum Peakwidth", "ppm", "S/N Threshold", "m/z Difference", "Prefilter 1", "Prefilter 2", "bandwidth")))

parameters.ppm <- matrix(data = preset, ncol = 8, nrow = length(ppm), byrow = TRUE, dimnames = list(NULL, 
                         c("Minimum Peakwidth", "Maximum Peakwidth", "ppm", "S/N Threshold", "m/z Difference", "Prefilter 1", "Prefilter 2", "bandwidth")))

parameters.snthresh <- matrix(data = preset, ncol = 8, nrow = length(snthresh), byrow = TRUE, dimnames = list(NULL, 
                              c("Minimum Peakwidth", "Maximum Peakwidth", "ppm", "S/N Threshold", "m/z Difference", "Prefilter 1", "Prefilter 2", "bandwidth")))

parameters.mzdiff <- matrix(data = preset, ncol = 8, nrow = length(mzdiff), byrow = TRUE, dimnames = list(NULL, 
                            c("Minimum Peakwidth", "Maximum Peakwidth", "ppm", "S/N Threshold", "m/z Difference", "Prefilter 1", "Prefilter 2", "bandwidth")))

parameters.prefilter.scan.number <- matrix(data = preset, ncol = 8, nrow = length(prefilter.scan.number), byrow = TRUE, dimnames = list(NULL, 
                                           c("Minimum Peakwidth", "Maximum Peakwidth", "ppm", "S/N Threshold", "m/z Difference", "Prefilter 1", "Prefilter 2", "bandwidth")))

parameters.prefilter.scan.abundance <- matrix(data = preset, ncol = 8, nrow = length(prefilter.scan.abundance), byrow = TRUE, dimnames = list(NULL, 
                                              c("Minimum Peakwidth", "Maximum Peakwidth", "ppm", "S/N Threshold", "m/z Difference", "Prefilter 1", "Prefilter 2", "bandwidth")))

parameters.bandwidth <- matrix(data = preset, ncol = 8, nrow = length(bandwidth), byrow = TRUE, dimnames = list(NULL, 
                                              c("Minimum Peakwidth", "Maximum Peakwidth", "ppm", "S/N Threshold", "m/z Difference", "Prefilter 1", "Prefilter 2", "bandwidth")))
...
results.peakwidth.min <- cbind(parameters.peakwidth.min, matrix(nrow = length(peakwidth.min), ncol = 7, dimnames = list(NULL, c("cvmin", "cv0.25", "cvmed", "cv0.75", "cvmax", "cviqr", "time"))))
results.peakwidth.max <- cbind(parameters.peakwidth.max, matrix(nrow = length(peakwidth.max), ncol = 7, dimnames = list(NULL, c("cvmin", "cv0.25", "cvmed", "cv0.75", "cvmax", "cviqr", "time"))))
results.ppm <- cbind(parameters.ppm, matrix(nrow = length(ppm), ncol = 7, dimnames = list(NULL, c("cvmin", "cv0.25", "cvmed", "cv0.75", "cvmax", "cviqr", "time"))))
results.snthresh <- cbind(parameters.snthresh, matrix(nrow = length(snthresh), ncol = 7, dimnames = list(NULL, c("cvmin", "cv0.25", "cvmed", "cv0.75", "cvmax", "cviqr", "time"))))
results.mzdiff <- cbind(parameters.mzdiff, matrix(nrow = length(mzdiff), ncol = 7, dimnames = list(NULL, c("cvmin", "cv0.25", "cvmed", "cv0.75", "cvmax", "cviqr", "time"))))
results.prefilter.scan.number <- cbind(parameters.prefilter.scan.number, matrix(nrow = length(prefilter.scan.number), ncol = 7, dimnames = list(NULL, c("cvmin", "cv0.25", "cvmed", "cv0.75", "cvmax", "cviqr", "time"))))
results.prefilter.scan.abundance <- cbind(parameters.prefilter.scan.abundance, matrix(nrow = length(prefilter.scan.abundance), ncol = 7, dimnames = list(NULL, c("cvmin", "cv0.25", "cvmed", "cv0.75", "cvmax", "cviqr", "time"))))
results.bandwidth <- cbind(parameters.bandwidth, matrix(nrow = length(bandwidth), ncol = 7, dimnames = list(NULL, c("cvmin", "cv0.25", "cvmed", "cv0.75", "cvmax", "cviqr", "time"))))
...
for(i in 1:length(peakwidth.min)) {
  ...
  set <- xcmsSet(files ,method = "centWave", 
                 peakwidth = c(peakwidth.min[i],preset[2]), ppm = preset[3], snthresh = preset[4], 
                 mzdiff= preset[5], prefilter = c(preset[6], preset[7]), BPPARAM = params)
  ...
  #Filter features by retention time (only features between 2 and 12 min) and abundance (only feature higher E6)
...
results.peakwidth.min <- arrange(as.data.frame(results.peakwidth.min), cvmed, cviqr, time)
...
for(i in 1:length(peakwidth.max)) {
  ...
  #Preprocessing of raw files
  set <- xcmsSet(files ,method = "centWave", 
                 peakwidth = c(preset[1],peakwidth.max[i]), ppm = preset[3], snthresh = preset[4], 
                 mzdiff= preset[5], prefilter = c(preset[6], preset[7]), BPPARAM = params)
  ...
  #Summarising cv distribution in results table
  results.peakwidth.max[i, 9] <- round(min(peaklist.modified.filtered$abundance.cv), 0)
  results.peakwidth.max[i, 10] <- round(quantile(peaklist.modified.filtered$abundance.cv, probs = 0.25), 0)
  results.peakwidth.max[i, 11] <- round(median(peaklist.modified.filtered$abundance.cv), 0)
  results.peakwidth.max[i, 12] <- round(quantile(peaklist.modified.filtered$abundance.cv, probs = 0.75), 0)
  results.peakwidth.max[i, 13] <- round(max(peaklist.modified.filtered$abundance.cv), 0)
  results.peakwidth.max[i, 14] <- round(results.peakwidth.max[i, 11] - results.peakwidth.max[i, 9], 0)
  
  #Recording time at end of loop and add difference to results table
  end.time <- Sys.time()
  results.peakwidth.max[i, 15] <- round(as.numeric(end.time) - as.numeric(start.time),0)
}
results.peakwidth.max <- arrange(as.data.frame(results.peakwidth.max), cvmed, cviqr, time)
...
for(i in 1:length(ppm)) {
  ...
  #Preprocessing of raw files
  set <- xcmsSet(files ,method = "centWave", 
                 peakwidth = c(preset[1],preset[2]), ppm = ppm[i], snthresh = preset[4], 
                 mzdiff= preset[5], prefilter = c(preset[6], preset[7]), BPPARAM = params)
...
for(i in 1:length(snthresh)) {
  ...
  #Preprocessing of raw files
  set <- xcmsSet(files ,method = "centWave", 
                 peakwidth = c(preset[1],preset[2]), ppm = preset[3], snthresh = snthresh [i], 
                 mzdiff= preset[5], prefilter = c(preset[6], preset[7]), BPPARAM = params)
...
for(i in 1:length(mzdiff)) {
  ...
  #Preprocessing of raw files
  set <- xcmsSet(files ,method = "centWave", 
                 peakwidth = c(preset[1],preset[2]), ppm = preset[3], snthresh = preset[4], 
                 mzdiff= mzdiff[i], prefilter = c(preset[6], preset[7]), BPPARAM = params)
...
for(i in 1:length(prefilter.scan.number)) {
  ...
  set <- xcmsSet(files ,method = "centWave", 
                 peakwidth = c(preset[1],preset[2]), ppm = preset[3], snthresh = preset[4], 
                 mzdiff= preset[5], prefilter = c(prefilter.scan.number[i], preset[7]), BPPARAM = params)
...
for(i in 1:length(prefilter.scan.abundance)) {
  ...
  set <- xcmsSet(files ,method = "centWave", 
                 peakwidth = c(preset[1], preset[2]), ppm = preset[3], snthresh = preset[4], 
                 mzdiff= preset[5], prefilter = c(preset[6], prefilter.scan.abundance[i]), BPPARAM = params)
...
for(i in 1:length(bandwidth)) {
  ...
  set <- xcmsSet(files ,method = "centWave", 
                 peakwidth = c(preset[1], preset[2]), ppm = preset[3], snthresh = preset[4], 
                 mzdiff= preset[5], prefilter = c(preset[6], prefilter.scan.abundance[7]), BPPARAM = params)
...
parameters.comb <- expand.grid(peakwidth.min.opt, peakwidth.max.opt, ppm.opt, snthresh.opt, mzdiff.opt, prefilter.scan.number.opt, prefilter.scan.abundance.opt, bandwidth.opt)
colnames(parameters.comb) <- c("Minimum Peakwidth", "Maximum Peakwidth", "ppm", "S/N Threshold", "m/z Difference", "Prefilter 1", "Prefilter 2", "bandwidth")

results.comb <- cbind(parameters.comb, matrix(nrow = length(parameters.comb[,1]), ncol = 7, dimnames = list(NULL, c("cvmin", "cv0.25", "cvmed", "cv0.75", "cvmax", "cviqr", "time"))))
...
for(i in 1:length(parameters.comb[,1])) {
  ...
  #Preprocessing of raw files
  set <- xcmsSet(files ,method = "centWave", 
                 peakwidth = c(parameters.comb[i,1],parameters.comb[i,2]), ppm = parameters.comb[i,3], snthresh = parameters.comb[i,4], 
                 mzdiff= parameters.comb[i,5], prefilter = c(parameters.comb[i,6], parameters.comb[i,7]), BPPARAM = params)
